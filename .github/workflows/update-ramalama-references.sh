...
IMAGES=(
  ...
  "llamacpp_cuda:ramalama/cuda-llama-server:cuda"
...
for entry in "${IMAGES[@]}"; do
  ...
  digest=$(curl -s "https://quay.io/v2/$image/manifests/$TAG"  -H 'Accept: application/vnd.oci.image.index.v1+json' --head | grep -i Docker-Content-Digest | awk -e '{ print $2 }' | sed 's/\\r//g')
